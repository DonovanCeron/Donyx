//pasar a binario num.toString(2);
function subneteo(ipv, nsubr){
    ...
    eliminarT();
    //Obtengo la direccion ip y el numero de subredes 
    var dir_ip = ipv;  
    var subr = nsubr;

    //Validar ip y num de subredes
    
    //El primer octeto de la direccion para determinar la clase
    ...
    //Poner de donde y hasta donde va cada subred
    //esta variable es 2 a la n para mi for
    var npow = Math.pow(2, n);
    let pos = 0;
    if(clase=="Clase A"){
        //Calcular el num de host
        var bit_libres = 24-n;
        var host = Math.pow(2, bit_libres)-2;
        //Nueva mascara en binario
        var ceros= 8-n;
        var cero = "0";
        var uno = "1";
        ...
        for(i=0; i<npow; i++){
            ...
            //Vamos a agregar al html la tablita uwu
...
function validarip(){
    ...
    if(subr>128){
        alert("El maximo de sub redes es 128 >:c");
        return false;
    }else if(ipv[0]>=192){
        if(subr >= 65){
            alert("Las clase C tienen un maximo de 64 subredes, con 7 ya no hay host");
            return false;
        }
    }else if(isNaN(subr)==true){
        alert("Numero de subredes vacio. No se puede Calcular >:c");
        return false;
    }else{
...
function subneteohost(){
    ...
    //Obtenemos la cantidad de bits robados con el num de host maximo y los que se quieren
    var oct1 = ipv[0];  
    var bitr = 0;   
    if(oct1 <= 127){
        //Clase A
    ...
    //Vamos a calcular el numero de subredes con los bit robados
    var subr = Math.pow(2, bitr);
    //Pasamos de nevo a String la ip
    ipv[0]=ipv[0].toString();
    ipv[1]=ipv[1].toString();
    ipv[2]=ipv[2].toString();
    ipv[3]=ipv[3].toString();
